...
class sitePage extends front{
	...
	function _getPlugMenu(){
		...
		if ($plugmenus){
				...
				switch ($pm['name']){
					case 'tel':
						if (!$pm['url']){
							...
							$pm['url']='http://site.tg.qq.com/forwardToPhonePage?siteId=1&phone={wbs:$thisCompany.tel}'.$pm['url'];
						}
						break;
					case 'memberinfo':
						if (!$pm['url']){
							$pm['url']='/index.php?wbs=Wap&cms=Userinfo&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
						}
						break;
					case 'nav':
						if (!$pm['url']){
							$pm['url']='/index.php?wbs=Wap&cms=Company&a=map&token='.$this->token.'&wecha_id='.$this->wechat_id;
						}
						break;
					case 'message':
						break;
					...
					case 'home':
						if (!$pm['url']){
							$pm['url']='/index.php?wbs=Wap&cms=Index&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
						}
						break;
					case 'album':
						if (!$pm['url']){
							$pm['url']='/index.php?wbs=Wap&cms=Photo&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
						}
						break;
					case 'email':
						$pm['url']='email:'.$pm['url'];
					...
					case 'shopping':
						if (!$pm['url']){
							$pm['url']='/index.php?wbs=Wap&cms=Product&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
						}
						break;
					case 'membercard':
						...
						if (!$pm['url']){
							if($card==false){
								$pm['url']='/index.php?wbs=Wap&cms=Card&a=get_card&token='.$this->token.'&wecha_id='.$this->wechat_id;
	
							}else{
								$pm['url']='/index.php?wbs=Wap&cms=Card&a=vip&token='.$this->token.'&wecha_id='.$this->wechat_id;
			...
			/*
			$plugmenus=array(
			array('name'=>'home','url'=>'/index.php?wbs=Wap&cms=Index&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id),
			array('name'=>'nav','url'=>'/index.php?wbs=Wap&cms=Company&a=map&token='.$this->token.'&wecha_id='.$this->wechat_id),
...
marker1.addEventListener("click", function(){this.openInfoWindow(infoWindow1);});
	...
	public function getLink($url){
		...
		if (strExists($url,'刮刮卡')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Guajiang&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link.='&id='.$itemid;
			}
		}elseif (strExists($url,'大转盘')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Lottery&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link.='&id='.$itemid;
			}
		}elseif (strExists($url,'优惠券')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Coupon&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link.='&id='.$itemid;
			}
		}elseif (strExists($url,'商家订单')){
			if ($itemid){
				$link=$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Host&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&hid='.$itemid;
			}else {
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Host&a=Detail&token='.$this->token.'&wecha_id='.$this->wechat_id;
			}
		}elseif (strExists($url,'万能表单')){
			if ($itemid){
				$link=$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Selfform&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}else {
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Host&a=Detail&token='.$this->token.'&wecha_id='.$this->wechat_id;
			}
		}elseif (strExists($url,'相册')){
			if ($itemid){
				$link=$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Photo&a=plist&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}else {
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Photo&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			}
		}elseif (strExists($url,'会员卡')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Card&a=vip&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'商城')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Product&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'订餐')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Product&a=dining&dining=1&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'团购')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Groupon&a=grouponIndex&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'网站分类')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Index&a=lists&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Index&a=lists&token='.$this->token.'&wecha_id='.$this->wechat_id.'&classid='.$itemid;
			}
		}elseif (strExists($url,'图文回复')){
			if ($itemid){
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Index&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}elseif (strExists($url,'LBS信息')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Company&a=map&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Company&a=map&token='.$this->token.'&wecha_id='.$this->wechat_id.'&companyid='.$itemid;
			}
		}elseif (strExists($url,'DIY宣传页')){
			$link=WBSCMS_URL.'/index.php/show/'.$this->token;
		}elseif (strExists($url,'婚庆喜帖')){
			if ($itemid){
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Wedding&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}elseif (strExists($url,'投票')){
			if ($itemid){
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Vote&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}elseif (strExists($url,'全景')){
			$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Panorama&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link=WBSCMS_URL.'/index.php?wbs=Wap&cms=Panorama&a=item&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}else {
			$link=str_replace(array('{wechat_id}','{siteUrl}'),array($this->wechat_id,WBSCMS_URL),$url);
		}
		return $link;
	}
