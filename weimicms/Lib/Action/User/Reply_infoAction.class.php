...
class Reply_infoAction extends UserAction{
	...
	public function set(){
		...
		if(IS_POST){
			...
			if ($row['infotype']=='Hotels'){//订房
				$phone_authorize=intval($_POST['phone_authorize']);
				
				if (isset($_POST['phone_authorize'])){
					$row['phone_authorize']=intval($_POST['phone_authorize']);
				}else {
					$row['phone_authorize']=0;
				}
