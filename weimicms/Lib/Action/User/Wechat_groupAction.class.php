...
class Wechat_groupAction extends UserAction {
    ...
    public function index() {
        ...
        $groups = $wechat_group_db->order('id ASC')->select();
        ...
        if ($list) {
            ...
            foreach ($list as $item) {
                $list[$i]['bingshi_pic']=json_decode($item['bingshi_pic'],true);
    ...
    public function detail(){
        $id=$_GET['id'];
        $user=M("Wechat_group_list")->where("id=$id")->find();
        $user['bingshi_pic']=json_decode($user['bingshi_pic'],true);
    ...
    public function guanzhu() {
        $group_list_db = M('Wechat_group_list');
        $where = array('token' => $this->token, 'status' => 1, 'state' => 1);
        if (IS_POST && strlen(trim($_POST['keyword']))) {
            $keyword = htmlspecialchars(trim($_POST['keyword']));
            $where['nickname'] = array('like', '%' . $keyword . '%');
            $list = $group_list_db->where($where)->order('id desc')->select();
    ...
    public function zhuce() {
        $group_list_db = M('Wechat_group_list');
        $where = array('token' => $this->token, 'status' => 2);
        if (IS_POST && strlen(trim($_POST['keyword']))) {
            $keyword = htmlspecialchars(trim($_POST['keyword']));
            $where['nickname'] = array('like', '%' . $keyword . '%');
            $list = $group_list_db->where($where)->order('id desc')->select();
        ...
        if ($list) {
            $i = 0;
            foreach ($list as $item) {
            	if(substr($item['headimgurl'], 0, 1)=="/"){
            		$list[$i]['smallheadimgurl'] = 'http://bl.shopst.com'.$item['headimgurl'];
            	}else{
	                $t = substr($item['headimgurl'], 0, -1);
	                $list[$i]['smallheadimgurl'] = $item['headimgurl'];
            	}
    ...
    public function send() {
        if (IS_GET) {
            ...
            if (strlen($nextOpenID)) {
                $this->success('本次更新' . $a . '条,重复' . ($b = $b == 1 ? 0 : $b . '条，正在获取下一批用户数据'), '?g=User&m=Wechat_group&a=send&token=' . $this->token . '&next_openid=' . $nextOpenID);
            } else {
                $this->success('更新完成,现在获取用户详细信息', '?g=User&m=Wechat_group&a=send_info&token=' . $this->token);
            }
        } else {
            $this->error('非法操作');
        }
    ...
    public function send_info() {
        if (IS_GET) {
            ...
            if ($refreshAll) {
                ...
                if ($fans) {
                    foreach ($fans as $data_all) {
                        ...
                        if ($classData->subscribe == 1) {
                            ...
                            $nickname = M('wechat_group_list')->field("nickname")->where("openid='{$data_all['openid']}'")->find();
                            if (!$nickname['nickname']) {
                                $arr = array("nickname" => "保联公社社员");
                            }
                            M('wechat_group_list')->where("openid='{$data_all['openid']}'")->save($arr);
                ...
                $this->success('还有' . $count . '个用户信息没有更新,<br />请耐心等待', U('Wechat_group/send_info'));
            }
        } else {
            $this->error('非法操作');
        }
    ...
    public function sysGroups()
    {
        ...
        $this->success('操作成功', U('Wechat_group/groups',array('name'=>"Grouping")));
    }
