...
class UsersAction extends BaseAction{
	...
	public function checklogin(){
		...
		if($res&&($pwd===$res['password'])){
			...
			if($res['status']==0){
				$this->error('该账号已关闭，请联系超级管理员');exit;
			}
			...
			$wxuser=M('Wxuser')->field('token')->find();
			session('token',$wxuser['token']);
			$this->success('登录成功',U('User/Index/index',array('token'=>$wxuser['token'])));
		}else{
			$this->error('帐号密码错误',U('Login/index'));
		}
	}
	...
	public function sendMsg(){
		...
		if(IS_POST){
			...
			require('./weimicms/Lib/ORG/RestSMS.php');
