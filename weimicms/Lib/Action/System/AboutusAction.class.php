...
class AboutusAction extends BackAction
{
    public function index()
    {
        ...
        if (!$nodeExist) {
            $row2 = array('name' => 'add', 'title' => '添加', 'status' => 1, 'remark' => '0', 'pid' => $firstNode['id'], 'level' => 3, 'sort' => 0, 'display' => 2);
            M('Node')->add($row2);
        }
        $map = array();
        $UserDB = D('Funintro');
    ...
    public function add()
    {
        if (IS_POST) {
            $_DB = M('Funintro');
            $_DB->add($_POST);
            $this->success('添加成功', U('Aboutus/index', array('token' => session('token'), 'type' => 1)));
            die;
        } else {
    ...
    public function edit()
    {
        if (IS_POST) {
            $_DB = M('Funintro');
            $ok = $_DB->save($_POST);
            $this->success('修改成功', U('Aboutus/index', array('token' => session('token'), 'type' => 1)));
            die;
        } else {
