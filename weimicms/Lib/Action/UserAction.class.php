...
class UserAction extends BaseAction{
	...
	protected function _initialize(){
		...
		$userinfo=M('bl_group')->where(array('id'=>session('hid')))->find();
		$this->assign('userinfo',$userinfo);
		$this->userGroup=$userinfo;
		...
		if (session('hid')==1) {
			//超管
			$Nodedb=M('bl_node');
			$list=$Nodedb->where(array('pid'=>1,'level'=>0,'display'=>1))->order('sort ASC,id ASC')->select();
			$menus=array();
			foreach ($list as $val){
				$subs=array();
				$clist=$Nodedb->where(array('pid'=>$val['id'],'level'=>2,'display'=>1))->order('sort ASC,id ASC')->select();
				foreach ($clist as $v){
					$subs[]=array(
							'name'=>$v['title'],
							'Nname'=>$v['name'],
							'link'=>U($v['href'],array('token'=>$this->token,'name'=>$v['name'])),
							'new'=>0,
							'selectedCondition'=>array('m'=>$v['name'])
					);
				}
				$menus[]=array(
						'name'=>$val['title'],
			...
			/* $model=MODULE_NAME;
			$thisnode=M('bl_node')->where(array('name'=>$model))->find();
			$thisacc=M('bl_access')->where(array('role_id'=>session('hid'),'node_id'=>$thisnode['id']))->find();
			if (!$thisacc&&$model!=='Index') {
				$this->error('对不起，您没有权限进行次操作！');
			} */
			$Nodedb=M('bl_node');
			$list=$Nodedb->where(array('pid'=>1,'level'=>0,'display'=>1))->order('sort ASC,id ASC')->select();
			$menus=array();
			foreach ($list as $key=>$val){
				$res1=M('bl_access')->where(array('role_id'=>session('hid'),'node_id'=>$val['id']))->find();
				if ($res1) {
					$subs=array();
					$clist=$Nodedb->where(array('pid'=>$val['id'],'level'=>2,'display'=>1))->order('sort ASC,id ASC')->select();
					foreach ($clist as $k=>$v){
						$res2=M('bl_access')->where(array('role_id'=>session('hid'),'node_id'=>$v['id']))->find();
						if ($res2) {
							$subs[]=array(
								'name'=>$v['title'],
								'Nname'=>$v['name'],
								'link'=>U($v['href'],array('token'=>$this->token)),
								'new'=>0,
								'selectedCondition'=>array('m'=>$v['name'])
							);
						}else{
...
// 		$trans = include('./weimicms/Lib/ORG/FuncToModel.php');
