...
class Common_util_pub
{
	...
	public function getInfo_a($url){
	    $url = $this->createOauthUrlForCode(urlencode(C("site_url").$url));
	    Header("Location:$url"); exit();
	}
	function createOauthUrlForCode($redirectUrl)
	{
	    $urlObj["appid"] = $this->wxuser['appid'];
	...
	//获取jsapi_ticket（微信分享用到的ticket）
	function wx_get_jsapi_ticket(){
	    // jsapi_ticket 应该全局存储与更新，以下代码以写入到文件中做示例
	    //$data = json_decode($this->get_php_file("jsapi_ticket.php"));
	    $data=M("Bl_jsapi_ticket")->where("id=1")->find();
	    if (($data['createtime']+3600)>time()) {
	        ...
	        $ticket = $res->ticket;
	        if ($ticket) {
	            M("Bl_jsapi_ticket")->where("id=1")->save(array("jsapi_ticket"=>$ticket,"createtime"=>time()));
	        } else {
	            $this->error('获取jsapi_ticket发生错误：错误代码' . $res->errcode . ',微信返回错误信息：' . $res->errmsg);
	        }
	        return $ticket;
	    }
	...
	public function getInfo($url){
	    //=========步骤1：网页授权获取用户openid============
	    //触发微信返回code码
	    $url = $this->createOauthUrlForCodes(urlencode(C("site_url").$url));
	    Header("Location:$url"); exit();
	
	    //获取code码，以获取openid
	}
	public function getInfo_pc($url){
	    //=========步骤1：网页授权获取用户openid============
	    //触发微信返回code码
	    $url = $this->createOauthUrlForCodes(urlencode(C("site_url").$url));
	    return $url;
	...
	function createOauthUrlForCodes($redirectUrl)
	{
	    $urlObj["appid"] = $this->wxuser['appid'];
	    $urlObj["redirect_uri"] = "$redirectUrl";
	    $urlObj["response_type"] = "code";
	...
	{
	    $urlObj["appid"] = $this->wxuser['appid'];
	    $urlObj["secret"] = $this->wxuser['appsecret'];
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT,'/var/www/bl/weimicms/Lib/ORG/Wxpay/cert/apiclient_cert.pem');
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, '/var/www/bl/weimicms/Lib/ORG/Wxpay/cert/apiclient_key.pem');
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
	...
	function createXml()
	{
		try
		{
				...
				$this->parameters["transaction_id"] == null) 
			{
		   	...
		   	$this->parameters["appid"] = $ths->appid;//公众账号ID
		   	$this->parameters["mch_id"] = $this->mchid;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
