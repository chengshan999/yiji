...
<div class="content-center ">
    ...
    <div class="center-right-center f-l" style="width:100%">
        <div class="center-right-order-name" >确认订单</div>
        <div class="border1"></div>
        <!--填写订单 begin-->
        
        <div class="orders-content share  ">
            <div class="gwc-orders">
                <h3>选择收货地址</h3>
                <?php if(is_array($shouhuo)): foreach($shouhuo as $key=>$v): if($v[selected] == 1): ?><div class="gwc-orders-add orders-add">
                <?php else: ?>
                <div class="gwc-orders-add"><?php endif; ?>
                <input type="hidden" name="shouhuo_id" value="<?php echo ($v["id"]); ?>" />
                    <p >（<?php echo ($v["xingming"]); ?>收）<?php echo ($v["province"]); ?>&nbsp;<br /><?php echo ($v["city"]); ?>&nbsp;<?php echo ($v["area"]); ?></p>
                    <p style="border-top:1px solid #eee;padding-top:10px;"><?php echo ($v["addr"]); ?> <br />
                        <?php echo ($v["tel"]); ?></p>
                </div><?php endforeach; endif; ?>
            </div>
            <div style="clear: both"></div>
            <div class="border"></div>
            <div class="gwc-orders-tianxie">
                <div class="gwc-orders-dis">
                    <div class="order-1">
                        商品
                    </div>
                    <div class="order-2">
                    ...
                    <div class="order-3 number">
                        数量
                    </div>
                    <div class="order-4">
                        合计
                    </div>
                    <div class="order-4 del">

                    </div>
                </div>
                <?php if(is_array($product)): foreach($product as $key=>$v): ?><div class="order-xiangqing">
                    <div class="order-4"><img style="width:73%;" src="<?php echo ($v["pic"]); ?>" alt="" class="img-responsive"></div>
                    <div class="order-2">
                        <?php echo ($v["productName"]); ?>
                    </div>
                    <div class="order-4" style="text-indent: -50px;"><?php echo ($v["color"]); ?> &nbsp; <?php echo ($v["guige"]); ?></div>
                    <div class="order-3">¥<?php echo ($v["price"]); ?></div>
                    <div class="order-3 number">
                        <span><?php echo ($v["number"]); ?></span>
                    </div>
                    <div class="order-4">
                        ¥<?php echo ($v["total_price"]); ?><br/><span>(含运费:¥<?php echo ($v["kuaidi_price"]); ?>.00)</span>
                    </div>
                    <?php if(count($product)-1 == $key): ?><div class="order-4 del">
                    	总计：¥<?php echo ($total_price); ?>
                        <a onclick="tijiao()" style="cursor:pointer" >提交订单</a>
                    </div><?php endif; ?>
                </div>
...
<script>
	function tijiao(){
		var shouhuo_id=$(".orders-add input").val();
		location.href="__URL__/order_save/order_id/<?php echo ($order["id"]); ?>/shouhuo_id/"+shouhuo_id;
	}
</script>