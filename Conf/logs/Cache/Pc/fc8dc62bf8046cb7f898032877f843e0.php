...
<div class="content-center ">
    ...
    <div class="center-right-center f-l contrnt-zf">
        <div class="center-right-order-name">支付页面</div>
        ...
        <div class="gwc-orders-tianxie">
            ...
            <?php if(is_array($product)): foreach($product as $key=>$v): ?><div class="order-xiangqing">
                <div class="order-4">
                    <img style="width:73%" src="<?php echo ($v["pic"]); ?>" alt="" class="img-responsive">
                </div>
                <div class="order-2">
                    <?php echo ($v["productName"]); ?>
                </div>
                <div class="order-4"><?php echo ($v["color"]); ?> &nbsp; <?php echo ($v["guige"]); ?></div>
                <div class="order-4">¥<?php echo ($v["price"]); ?></div>
                <div class="order-4 number">
                    <span><?php echo ($v["number"]); ?></span>
                </div>
                <?php if(count($product)-1 == $key): ?><div class="order-2">
                    ¥<?php echo ($v["total_price"]); ?><br/><span>(含运费:¥<?php echo ($v["kuaidi_price"]); ?>.00)</span><br /><br />
                    订单总价：¥<?php echo ($order["total_price"]); ?><br/><span>(含运费:¥<?php echo ($kuaidi_price_all); ?>.00)</span>
                </div>
                <?php else: ?>
                <div class="order-2">
        ...
        <form action="" class="zf-ways">
			<?php if($jingxiaoshang != 1): ?><div class="zf-right">
                <input type="radio" name="rd" id="chat-zf" value="1">
                <label for="chat-zf" class="label">
                    <img src="/tpl/Pc/default/common/images/zfb.png" alt="" class="img-responsive">
                </label>

            </div>

            <div class="zf-right">
                <input type="radio" name="rd" id="chat-wx" value="2">
                <label for="chat-wx" class="label">
                    <img src="/tpl/Pc/default/common/images/wxzf.png" alt="" class="img-responsive">
                </label>
            </div><?php endif; ?>
            <div class="zf-right">
                <input type="radio" name="rd" id="chat-xx" value="3">
                <label for="chat-xx" class="label">
                    <img src="/tpl/Pc/default/common/images/xxzf.jpg" alt="" class="img-responsive">
                </label>

            </div>
            <div style="clear: both"></div>
            <div class="zf-sure">
                <a onclick="zhifu()" style="cursor:pointer" class="zf-sure-a">确认</a>
            </div>
...
<script>
var grade="<?php echo ($jingxiaoshang); ?>";
if(grade==1){
	$(".zf-right label").click();
}else{
	$(".zf-right:eq(1) label").click();
}

function zhifu(){
	var order_id="<?php echo $_GET['order_id'];?>";
	var zhifu_type=$("input[name=rd]:checked").val();
	if(zhifu_type==1){
		$.ajax({
			...
			success:function(html){
				$(".center-right-center").append(html);
				$("#zhifubao").submit();
			}
		})
	}else if(zhifu_type==2){
		location.href="__URL__/wxpay/order_id/"+order_id;
	}else if(zhifu_type==3){
		location.href="__URL__/xianxiazhifu/order_id/"+order_id;
	}
}
