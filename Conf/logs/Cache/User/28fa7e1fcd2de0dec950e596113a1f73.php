...
<div class="cLineB">
  <h4>推荐位管理 <span class="FAQ">咨讯推荐位轮播图</span></h4>
 </div>
         <div class="cLineC" style="line-height: 32px;">
 <a href="<?php echo U('Other/kefu_add',array('tip'=>$tip,'name'=>$name));?>" class="btnGrayS vm bigbtn" ><?php if($tip == 1): ?>添加轮播图<?php else: ?>添加图片<?php endif; ?></a> 
</div>
...
<thead>
<tr>
				   <th style=" width:60px;">客服名称</th>
                   <TH style=" width:60px;">客服头像</TH>
                   <th style=" width:120px;"><?php if($tip == 1): ?>描述<?php else: ?>图片描述<?php endif; ?></th>

<th>图片 <a  target="999" class="tooltips" ><img src="<?php echo RES;?>/images/price_help.png" align="absmiddle" /><span>
<p>如果不填写分类地址系统将随机分配默认。</p>
</span></a></th>
                        <th>图片外链地址<span class="tooltips" ><img src="<?php echo RES;?>/images/price_help.png" align="absmiddle" /><span>
...
<form action="<?php echo U('Flash/usort',array('tip'=>$info[0]['tip'],'name'=>$name));?>" method="post">
<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
<td><select name="udisplay[<?php echo ($vo["id"]); ?>]" >
	<?php if($vo["display"] == 1): ?><option value="1" selected="selected">推荐</option>
	<option value="0">不推荐</option>
	<?php else: ?>
	<option value="1" >推荐</option>
	<option value="0" selected="selected">不推荐</option><?php endif; ?>
</select></td>
<td><input type="number" style=" width:40px;border:1px solid #c9c9c9;-webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px;" class="usort" name="usort[<?php echo ($vo["id"]); ?>]" value="<?php echo ($vo["sort"]); ?>" /></td>
...
<td class="norightborder">
	<a href="<?php echo U('kefu_edit',array('id'=>$vo['id'],'tip'=>$tip,'name'=>$name));?>">编辑</a>
	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('kefu_del',array('id'=>$vo['id'],'tip'=>$tip,'name'=>$name));?>');">删除</a>
</td>
...
<?php if(!empty($info)): ?><input style="margin-left:5px" class="btnGrayS" type="submit" value="确认" /><?php endif; ?>
</form>
