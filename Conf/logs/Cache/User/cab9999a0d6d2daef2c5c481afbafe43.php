...
<input type="text" id="msgSearchInput" class="px" placeholder="输入订单号搜索" name="keyword" value="<?php echo ($keyword); ?>">
<input type="submit" value="搜索" id="msgSearchBtn" href="" class="btnGrayS" title="搜索">
</form>
</div>
<form method="post" action="<?php echo U('User/Wechat_group/setGroup');?>" id="info">
          <div style="width:70%;float:left">按订单状态查看：<select  onchange="location.href='?s=/User/Dingdan/index/name/Dingdan/status/'+this.value"><option value="">全部</option><?php if(is_array($status)): $i = 0; $__LIST__ = $status;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["id"]); ?>"<?php if ($_GET['status']==$g['id']){echo ' selected';}?>><?php echo ($g["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?></select>
          </div>
<div style="clear:both"></div>
</div>
...
<script src="<?php echo RES;?>/js/thickbox-compressed.js" type="text/javascript"></script>
<link href="<?php echo RES;?>/css/thickbox.css" media="screen" rel="stylesheet" type="text/css" />
    ...
    <div class="msgWrap bgfc">
	  <form class="form" method="post" action="" target="_top" enctype="multipart/form-data">	 
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="platform_table">
			...
			<?php if(is_array($data_tixing)): $n = 0; $__LIST__ = $data_tixing;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr>
					<td><?php echo ($vo['id']); ?></td>
					<td><?php echo ($vo['tel']); ?></td>
					<td><?php echo ($vo['orderid']); ?></td>
					<td><?php echo ($vo['total_price']); ?>(含运费) </td>
					<td><?php echo date("Y-m-d H:i:s",$vo['createtime']);?></td>
					<td>
					<font color="blue">待发货(<span style="color:red;">催货</span>)</font>
					</td>
					<td></td>
					<td><a href="index.php?g=User&m=Dingdan&a=select&id=<?php echo ($vo['id']); ?>"  class="thickbox" type="button" >查看</a></td>
					<td><?php if($vo['status'] != 0): ?><a href="index.php?g=User&m=Dingdan&a=fahuo&id=<?php echo ($vo['id']); ?>"  class="thickbox" type="button"">发货</a><?php endif; ?></td>
					<td><a href="index.php?g=User&m=Dingdan&a=query_wuliu&id=<?php echo ($vo['id']); ?>"  class="thickbox" type="button"">查看物流</a></td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			<?php if(is_array($data_wei_tixing)): $n = 0; $__LIST__ = $data_wei_tixing;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr>
					<td><?php echo ($vo['id']); ?></td>
					<td><?php echo ($vo['tel']); ?></td>
					<td><?php echo ($vo['orderid']); ?></td>
					<td><?php echo ($vo['total_price']); ?>(含运费) </td>
					<td><?php echo date("Y-m-d H:i:s",$vo['createtime']);?></td>
					<td>
					<?php if($vo['status'] == 1): ?><font color="blue">待发货</font>
					<?php elseif(($vo['status'] == 0) AND ($vo[zhifu_type] == '线下支付') AND ($vo[xianxia_status] == 0)): ?>线下支付(未支付)
					<?php elseif(($vo['status'] == 0) AND ($vo[zhifu_type] == '线下支付') AND ($vo[xianxia_status] == 1)): ?>已上传支付凭证<a target="_blank" href="<?php echo ($vo[zhifu_pic]); ?>">(查看)</a>
					<?php elseif($vo['status'] == 0): ?>未支付
					<?php elseif($vo['status'] == 2): ?>待收货
					<?php elseif($vo['status'] == 3): ?>待评价
					<?php elseif($vo['status'] == 4): ?>完成<?php endif; ?>
					</td>
					<td>
					<?php if(($vo['zhifu_type'] == '线下支付') AND ($vo[status] == 0) AND ($vo[xianxia_status] == 1)): ?><select name="change_xianxiazhifu_status" id="<?php echo ($vo[id]); ?>">
					<option value="">审核中</option>
					<option value="2">通过</option>
					<option value="3">驳回</option>
					</select>
					<?php elseif(($vo['zhifu_type'] == '线下支付') AND ($vo[status] == 0) AND ($vo[xianxia_status] == 2)): ?>
					已通过
					<?php elseif(($vo['zhifu_type'] == '线下支付') AND ($vo[status] == 0) AND ($vo[xianxia_status] == 3)): ?>
					已驳回<?php endif; ?>
					</td>
					<td><a href="index.php?g=User&m=Dingdan&a=select&id=<?php echo ($vo['id']); ?>"  class="thickbox" type="button" >查看</a></td>
					<td><?php if($vo['status'] != 0): ?><a href="index.php?g=User&m=Dingdan&a=fahuo&id=<?php echo ($vo['id']); ?>"  class="thickbox" type="button"">发货</a><?php endif; ?></td>
...
$("select[name=change_xianxiazhifu_status]").change(function(){
	...
	if(confirm("请仔细核对，一旦更变将无法更改")){
		$.ajax({
			url:"<?php echo U('Dingdan/change_xianxiazhifu_status');?>",
