...
<script src="/tpl/static/artDialog/plugins/iframeTools.js"></script>
 <div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4><?php if($tip == 1): ?>添加幻灯片<?php else: ?>添加图片<?php endif; ?></h4><a href="javascript:history.go(-1);"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div> 
<div class="msgWrap">
...
<TABLE class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellSpacing="0" cellPadding="0" width="100%">
  ...
  <TD><input type="text" class="px" id="url" value="<?php echo ($data["name"]); ?>" name="name" style="width:500px" > <br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword"><?php if($tip == 1): ?>图片描述：<?php else: ?>活动描述：<?php endif; ?></label></TH>
  <TD><input type="text" class="px" id="keyword" value="<?php echo ($data["info"]); ?>" name="info" style="width:500px" ><br />
                  30个字简短分类描述，可为空   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword"><?php if($tip == 1): ?>图片：<?php else: ?>图片：<?php endif; ?></label></TH>
  <TD><input type="text" class="px" id="img" value="<?php echo ($data["pic"]); ?>" name="pic" style="width:500px" >  <script src="/tpl/static/upyun.js?2013"></script><a href="###" onclick="chooseFile('img','<?php if($tip == 1): ?>focus<?php else: ?>background<?php endif; ?>')" class="a_upload">选择</a> <a href="###" onclick="upyunPicUpload('img',<?php if($tip == 1): ?>700,420<?php else: ?>640,1136<?php endif; ?>,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('img')">预览</a> 建议图片(<?php if($tip == 1): ?>宽640高450<?php else: ?>宽640高1136<?php endif; ?>)  </TD>
  <TD>&nbsp;</TD>
</TR>
...
// 	marker.enableDragging();
	...
	var removeMarker = function(e,ee,marker){
		...
		$.ajax({
			...
			data:{lt:lt,rt:rt,id:"<?php echo ($data["id"]); ?>"},
			type:"POST",
			async:false,
	...
	var txtMenuItem = [
		{
			...
			callback:function(a){
				...
				$.ajax({
					...
					data:{lt:a.lng,rt:a.lat,id:"<?php echo ($data["id"]); ?>"},
					type:"POST",
					async:false,
					success:function(data){
						num=data;
...
function time(val)
{
    ...
    for(var i = val;i<point_number-1;i++)
    {
    	...
    	$.ajax({
    		...
    		data:{id:"<?php echo ($data["id"]); ?>",num:i},
    		type:"POST",
    		async:false,
    		dataType:"json",
    		success:function(point){
    			...
    			walking.setSearchCompleteCallback(function (rs) {
    			    ...
    			    $.ajax({
			    		...
			    		traditional:true,
			    		data:{id:"<?php echo ($data["id"]); ?>",num:i,point_arr:JSON.stringify(chartData)},
			    		type:"POST",
			    		async:false,
			    		success:function(data){
			    		}
