...
<div style="clear:both"></div>
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			  ...
			  <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                  ...
                  <td><?php echo ($list["xingming"]); ?></td>
                  <td>
					<div class="cateimg">
						<a href="<?php echo ($list["headimgurl"]); ?>" target="_blank"><img src="<?php echo ($list["headimgurl"]); ?>" class="cateimg_small"  /></a></div>
					</td>
                  <td><?php echo ($list["tel"]); ?></td>
                  <td><?php echo ($list["question"]); ?></td>
                  <td><?php echo (date('Y-m-d H:i:s',$list["createtime"])); ?></td>
                  <?php if($list["zhiding"] == 1): ?><td><a href="javascript:void(0);" onClick="if(confirm('取消置顶？')) location='<?php echo U('Zixun/quxiao_zhiding',array('id'=>$list[id]));?>'"  ><span style="color:red;">取消置顶</span></a> &nbsp; <a href="index.php?g=User&m=Zixun&a=niwenwoda_detail&id=<?php echo ($list['id']); ?>"  class="thickbox" type="button"">查看详情</a> &nbsp; <a href="<?php echo U('Zixun/delete_niwenwoda',array('id'=>$list[id]));?>">删除</a></td>
                  <?php else: ?>
                  <td><a href="javascript:void(0);" onClick="if(confirm('确认置顶？')) location='<?php echo U('Zixun/zhiding',array('id'=>$list[id]));?>'"  >置顶</a> &nbsp; <a href="index.php?g=User&m=Zixun&a=niwenwoda_detail&id=<?php echo ($list['id']); ?>"  class="thickbox" type="button"">查看详情</a> &nbsp; <a href="<?php echo U('Zixun/delete_niwenwoda',array('id'=>$list[id]));?>">删除</a></td><?php endif; ?>
                 <!--  <td>
                  <?php if($list['status'] == 0): ?><select  class="status">
                  	<option value="0" selected>未解决</option>
                  	<option value="1" >已解决</option>
                  </select><?php endif; ?>
                  <?php if($list['status'] == 1): ?><select  class="status">
...
$(".status").change(function(){
	...
	var val=$(this).val();
	$.ajax({
		url:"<?php echo U('Zixun/changestatus');?>",
		data:{id:id,val:val},
		type:"POST",
