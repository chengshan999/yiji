...
<form method="post" action="<?php echo U('User/Wechat_group/setGroup',array('name'=>'Unregistered'));?>" id="info">
          <div style="width:70%;float:left">将选中用户转移到分组中 <select name="wechatgroupid"><?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["wechatgroupid"]); ?>"><?php echo ($g["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?></select> <input type="submit" value="转移" id="msgSearchBtn" href="" class="btnGrayS" title="转移">&nbsp;&nbsp;按分组查看用户：<select id="wechatgroupid" onchange="location.href='?g=User&m=Wechat_group&a=index&token=<?php echo ($token); ?>&name=Unregistered&wechatgroupid='+this.value"><?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["wechatgroupid"]); ?>"<?php if ($_GET['wechatgroupid']==$g['wechatgroupid']){echo ' selected';}?>><?php echo ($g["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?></select>
          </div>
