...
<script src="/tpl/static/artDialog/plugins/iframeTools.js"></script>
 <div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4><?php if($tip == 1): ?>编辑幻灯片<?php else: ?>编辑图片<?php endif; ?></h4><a href="javascript:history.go(-1);"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div> 
<div class="msgWrap">
  <form class="form" method="post"   action="<?php echo U('Flash/upsave',array('id'=>$id,'tip'=>$tip,'name'=>$name));?>" enctype="multipart/form-data" >
  <input type="hidden" name="tip" value="<?php echo ($tip); ?>" />
<TABLE class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellSpacing="0" cellPadding="0" width="100%">
  <TBODY><input type="hidden" name="id" value="<?php echo ($info["id"]); ?>"/>
<TR>
  <TH valign="top"><label for="keyword"><?php if($tip == 1): ?>图片描述：<?php else: ?>图片描述：<?php endif; ?></label></TH>
  <TD><input type="text" class="px" id="keyword" value="<?php echo ($info["info"]); ?>" name="info" style="width:500px" ><br />
                  30个字简短分类描述，可为空   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword"><?php if($tip == 1): ?>图片：<?php else: ?>图片图片：<?php endif; ?></label></TH>
  <TD><input type="text" class="px" id="img" value="<?php echo ($info["img"]); ?>" name="img" style="width:500px" >  <script src="/tpl/static/upyun.js?2013"></script> <a href="###" onclick="upyunPicUpload('img',<?php if($tip == 1): ?>700,420<?php else: ?>640,1136<?php endif; ?>,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('img')">预览</a> 建议图片(<?php if($tip == 1): ?>宽640高450<?php else: ?>宽375高281<?php endif; ?>)  </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword">图片链接地址 ：</label></TH>
  <TD><input type="text" class="px" id="url" value="<?php echo (htmlspecialchars_decode($info["url"])); ?>" name="url" style="width:300px" >  <br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword">选择分类 ：</label></TH>
  <TD><select name="tip" id="">
  <option  value="0">医生端</option>
  <option <?php if($info[tip]==1){echo 'selected';} ?> value="1">用户端</option>
  </select><br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
