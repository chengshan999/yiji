...
<div class="cLineB">
  <h4>提现查询 </h4>
   <div class="ftip" style="margin:10px 0">在此显示使用平台支付的订单列表，方便与管理员进行提现管理。</div> 
 </div>
...
<!--tab end-->    
    ...
    <div class="msgWrap bgfc">
	  <form class="form" method="post" action="<?php echo U('Tixian/fanxian',array('name'=>$name));?>"  enctype="multipart/form-data">	 
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="platform_table">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td><input type="checkbox" value="<?php echo $vo['id'];?>" class="fuxuan" name='llist[]'/></td>
					<td><input type='text' value="<?php echo ($vo['order']); ?>" name='order[]' readonly="readonly" style='width: 221px;'/></td>
					<td><input type='text'  value="<?php echo ($vo['name']); ?>" name='name[]' readonly="readonly" class="name"/></td>
					<td><input type='text'  value="<?php echo ($vo['jine']); ?>" name='jine[]' readonly="readonly" class="jine"/></td>
					<td><input type='text'  value="<?php echo ($vo['points']); ?>" name='points[]' readonly="readonly" class="jine" style="width:113px;"/></td>
					<td><input type='text' value="<?php echo ($vo['time']); ?>" name='time[]' readonly="readonly"/></td>
					<td><input type='text'  value="<?php echo ($vo['mes']); ?>" name='mes[]' readonly="readonly"/></td>
					<td><input type='hidden'  value="<?php echo ($vo['openid']); ?>" name='openid[]' class="time_input"/></td>
			...
			<td colspan="5" style="text-align:right;">
				<input type='submit'  style="color:#000;font-size:14px;padding:1px 24px;background:#4A98E0;"value='返现'/></span>
			</td>
			<td colspan="5" style="text-align:right;">
				<a href="<?php echo U('Tixian/chaxun',array('name'=>$name));?>"  style="color:#000;font-size:14px;padding:1px 24px;background:#4A98E0;">查询成功记录</a> </span>
			</td>
			</tr>
		</table>
		<?php session_start(); ?>	
        ...
        <script type="text/javascript" src="<?php echo RES;?>/js/datetime.js"></script>
</div>
</div>
</div>
