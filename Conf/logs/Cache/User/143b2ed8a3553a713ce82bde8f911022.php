...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4>修改资讯内容</h4><a href="javascript:history.go(-1);"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div>
         
          <form method="post"   action="<?php echo U('Img/edit',array('id'=>$info[id]));?>"  enctype="multipart/form-data" >
                ...
                <TR>
                  ...
                  <TD>
					<select name="classid" id="">
                  	<?php if(is_array($fenlei)): foreach($fenlei as $key=>$v): ?><option <?php if($v['id']==$info['classid']){echo 'selected';} ?> value="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); ?></option>
                  		<?php if(is_array($v["fenlei"])): foreach($v["fenlei"] as $key=>$val): ?><option <?php if($val['id']==$info['classid']){echo 'selected';} ?> value="<?php echo ($val["id"]); ?>"> &nbsp;&nbsp;&nbsp; ┗ ━ ━<?php echo ($val["name"]); ?></option><?php endforeach; endif; endforeach; endif; ?>
                  		
                  	</select>
                  	 </TD>
                </TR>
                <TR style="display:none;">
                	<TH valign="top"><label for="ppt">上传文件：</label></TH>
                  <TD><input type="file" name="ppt" id="ppt" />已有文件:<?php echo ($info["ppt"]); ?>（不传视为不变）</TD>
                </TR>
                <TR>
                  <TH valign="top"><label for="pic">封面图片地址：</label></TH>
                  <TD><input type="text" class="px"  name="pic" id="pic" value="<?php echo ($info["pic"]); ?>" style="width:580px;"  />  <script src="/tpl/static/upyun.js?<?php echo date('YmdHis',time());?>"></script><a href="###" onclick="upyunPicUpload('pic',700,420,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('pic')">预览</a>
                  </TD>
                </TR>
                <TR>
                  <TH><label for="type">资源类型（图像文字/视频）：</label></TH>
                  <TD><input type="text" class="px" id="type" value="<?php echo ($info["type"]); ?>"    name="type" style="width:580px;"> </TD>
                </TR> 
                <TR> 
                 ...
                 <TR>
                  ...
                  <TD><input type="text" class="px" id="url" value="<?php echo ($info["url"]); ?>" name="url" style="width:380px;">  </TD>
                </TR>
                <TR>
                  <TH></TH>
                  <TD><input type="submit" value="保存" name="sbmt" class="btnGreen left">　<a href="<?php echo U('Img/index',array('name'=>$name));?>"  class="btnGray vm">取消</a></TD>
                </TR>
              </TBODY>
            </TABLE>
...
<script>
var classid="<?php echo ($info["classid"]); ?>";
if(classid==16){
	$("select").parent().parent().nextAll().css('display','none');
	$("select").parent().parent().parent().find('TR:last').css('display','');
